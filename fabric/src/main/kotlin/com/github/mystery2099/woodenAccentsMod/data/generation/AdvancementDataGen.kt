...
class AdvancementDataGen(output: FabricDataOutput, registries: CompletableFuture<HolderLookup.Provider>) : FabricAdvancementProvider(output, registries) {
    override fun generateAdvancement(registryLookup: HolderLookup.Provider, consumer: Consumer<AdvancementHolder>) {
        ...
        val desk = Advancement.Builder.advancement().parent(basicComfort)
            .display(ModBlocks.oakDesk, Component.literal("A place to work"),
                Component.literal("Craft a desk"), null, AdvancementType.TASK, true, false, false)
        ...
        Advancement.Builder.advancement().parent(desk)
            .display(ModBlocks.oakDeskDrawer, Component.literal("Everything in its drawer"),
                Component.literal("Craft a desk drawer for your supplies"), null, AdvancementType.TASK, true, false, false)
        ...
        val counter = Advancement.Builder.advancement().parent(basicComfort)
            .display(ModBlocks.oakKitchenCounter, Component.literal("Room to cook"),
                Component.literal("Craft a kitchen counter or cabinet"), null, AdvancementType.TASK, true, false, false)
        ...
        Advancement.Builder.advancement().parent(counter)
            .display(ModBlocks.oakKitchenCabinet, Component.literal("Stock the kitchen"),
                Component.literal("Craft a kitchen cabinet for your ingredients"), null, AdvancementType.TASK, true, false, false)
    ...
    private fun inventoryChangedConditionsInTag(tag: TagKey<Block>, itemCount: MinMaxBounds.Ints = MinMaxBounds.Ints.ANY): Criterion<InventoryChangeTrigger.TriggerInstance> {
        val itemTag = ModBlockTags.getItemTagFrom(tag)
        return InventoryChangeTrigger.TriggerInstance.hasItems(
            ItemPredicate.Builder.item().of(itemTag).withCount(itemCount).build()
        )
    }
    ...
    private fun placedBlockInTagConditions(
        ...
        val states = StatePropertiesPredicate.Builder.properties().apply(stateBuilder)
        val blockPredicate = BlockPredicate.Builder.block().of(tag).setProperties(states)
    ...
    private fun <T> placedBlockInTagConditions(
        ...
        value: T
    ): Criterion<ItemUsedOnLocationTrigger.TriggerInstance> where T : Comparable<T>, T : net.minecraft.util.StringRepresentable = placedBlockInTagConditions(tag) {
    ...
    private fun placedBlockInTagConditions(
        ...
        value: Boolean
    ): Criterion<ItemUsedOnLocationTrigger.TriggerInstance> = placedBlockInTagConditions(tag) {
        exactMatch(property, value)
    }
