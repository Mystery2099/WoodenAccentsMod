...
The generator in build-in-blockbench.js runs in a fresh Java block project in Blockbench. Set globalThis.decorConcept to parquet, screen, planter, or shelf before running it. It creates geometry and textures; export each project and its PNGs afterward. compose-previews.py regenerates the comparison sheet from the saved screenshots and checks the exported texture references.
